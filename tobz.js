...
let _autosticker = JSON.parse(fs.readFileSync('./lib/helper/antisticker.json'))
...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        //const isAutoStikerOn = isGroupMsg ? _autostiker.includes(groupId) : false
...
${desc}`)
        ...
        case prefix+'nsfw':
            ...
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner Anang bot', id)
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin group!', id)
...
➸ *Jumlah Postingan:* ${status_count}`
        ...
        case prefix+'igstalk':
            ...
            console.log(istalk)
            try {
                const istalk2 = await axios.get('https://api.vhtear.com/igprofile?query=' + istalk + '&apikey=7728hhgsYRTtjah2837')
...
➸ *Total Rekaman:* ${recording}`
        ...
        case prefix+'':
            ...
            //const sigo = await axios.get(`https://mhankbarbar.tech/api/samisami?text=${que}`)
...
➸ *Wilayah:* +${region_name}`
        ...
        case prefix+'brainly':
            ...
            try {
                const resp = await get(`https://api.vhtear.com/branly?query=${querykbbia}&apikey=7728hhgsYRTtjah2837`)
                if (resp.data.error) return tobz.reply(from, resp.data.error, id)
...
╰────「 *A N A N G  B O T* 」───`, id)
        ...
        case prefix+'otogel':
            if (!isOwner) return tobz.reply(from, 'perintah ini hanya bisa di gunakan oleh owner anang!', id)
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            const togel = fetch('https://api.vhtear.com/togel&apikey=7728hhgsYRTtjah2837')
        ...
        case 'bot?':
        case 'p':
            await tobz.reply(from, 'iyha.. ada apa sayangku..', id)
            break
        case 'iri':
        case 'Iri?':
        case prefix+'iri':
        ...
        case prefix+'anjing':
            ...
            tobz.sendPtt(from, './aris/astg.mp3', id)
            break
        case 'pale':
        case 'papepale':
        case prefix+'pale':
        ...
        /*case prefix+'sex':
            const cersex = () => new Promise((resolve, reject) => {
                ...
                fetch(`https://api.vhtear.com/cerita_sex&apikey=7728hhgsYRTtjah2837`)
        ...
        /*case prefix+'hm':
            ...
            break*/
        case prefix+'togel':
            const respon = await fetch('https://api.vhtear.com/togel&apikey=7728hhgsYRTtjah2837')
...
// By Gimenz
        ...
        case prefix+'bj18':
            ...
            .then(async (res) => await tobz.sendFileFromUrl(from, res.data.url,))
            .catch(async () => {
                await tobz.sendFileFromUrl(from, 'Error!', id)
            })
            /*.then(res => res.json())
            .then(body => {
        ...
        case prefix+'film':
            const datta = await fetchJson(`https://api.zeks.xyz/api/film?q=${body.slice(6)}&apikey=BotWeA`)
            const film = datta.result
            const fillm = `hasil ${film.title}\n${film.url}`
            await tobz.sendFileFromUrl(from, fillm, film.thumd, id)
            break
        case prefix+'javcosplay':
            ...
            await tobz.reply(from, mess.wait, id)
            const kont = await fetch('https://api.vhtear.com/nekojavcosplay&apikey=7728hhgsYRTtjah2837')
            const datas = await kont.json()
            const asuu = datas.result
            let foox = "*Neko JAV Cosplay*\n"
            for (var a = 0; a < asuu.length; a++) {
                foox += "\n\n" + asuu[a].detail
                foox += "*\n*Link :*\n" + asuu[a].url
            }
            tobz.reply(from, foox, id)
            break
        case prefix+'neko3d':
            if(isReg(obj)) return
            if(cekumur(cekage)) return
            ...
            await limitAdd(serial)
            //if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin Elaina', id)
            const asti = await fetch(`https://api.vhtear.com/neko3d&apikey=${vhtearkey}`)
            const fana = await asti.json()
            const nani = fana.result
            let foxx = "*Neko 3D*\n"
            for (var a = 0; a < nani.length; a++) {
                foxx += "\n\n*Title :*\n" + nani[a].title
                foxx += "\n*Detail :*\n" + nani[a].description
                foxx += "*\n*Link :*\n" + nani[a].url
            }
            tobz.reply(from, foxx, id)
            break
        case prefix+'fotocewek':
            ...
            tobz.sendFileFromUrl(from, cangtip, 'cewk.jpg')
            break
        case prefix+'quotesgambar':
            const aiquote = await axios.get("http://inspirobot.me/api?generate=true")
            await tobz.sendFileFromUrl(from, aiquote.data, 'quote.jpg', 'Powered By http://inspirobot.me/ With ❤️' , id )
            break
        case prefix+'infoloker':
            tobz.reply(from, mess.wait, id);
            const lokernya = await fetch('http://docs-jojo.herokuapp.com/api/infoloker')
            const lokr = await lokernya.json()
            tobz.reply(from, `➸ *Perusahaan* : ${lokr.data.result.perusahaan} \n➸ *Deskripsi* : ${lokr.data.result.desc} \n➸ *Edukasi* : ${lokr.data.result.edukasi} \n➸ *Lokasi* : ${lokr.data.result.lokasi} \n➸ *Gaji* : ${lokr.data.result.gaji} \n➸ *Syarat* : ${lokr.data.result.syarat} \n➸ *Pengalaman* : ${lokr.data.result.pengalaman} \n➸ *Level Karir* : ${lokr.data.result.levelKarir} \n➸ *Profesi* : ${lokr.data.result.profesi} \n➸ *Job Function* : ${lokr.data.result.jobFunction} \n➸ *Link* : ${lokr.data.result.link}`, id)
            //tobz.reply(from, `➸ *Perusahaan* : ${lokr.result.data.perusahaan} \n➸ *Deskripsi* : ${lokr.result.data.desc} \n➸ *Edukasi* : ${lokr.result.data.edukasi} \n➸ *Lokasi* : ${lokr.result.data.lokasi} \n➸ *Gaji* : ${lokr.result.data.gaji} \n➸ *Syarat* : ${lokr.result.data.syarat} \n➸ *Pengalaman* : ${lokr.result.datapengalaman} \n➸ *Level Karir* : ${lokr.result.data.levelKarir} \n➸ *Profesi* : ${lokr.result.data.profesi} \n➸ *Job Function* : ${lokr.data.result.jobFunction} \n➸ *Link* : ${lokr.data.result.link}`, id)
            break
        case prefix+'togeel':
            const totogel = await fetch(`https://api.vhtear.com/nekojavcosplay&apikey=${vhtearkey}`)
            const gelt = await totogel.json()
            const tgel = gelt.result.hasil
            let gel = '-----[ *INFO TOGEL* ]-----'
            for (var a = 0; a < tgel.length; a++) {
                gel += "\n\nNegara : " + tgel[a].Negara
                gel += "\nSenin : " + tgel[a].Senin
                gel += "\nSelasa : " + tgel[a].Selasa
                gel += "\nRabu : " + tgel[a].Rabu
                gel += "\nKamis : " + tgel[a].Kamis
                gel += "\nJumat : " + tgel[a].Jumat
                gel += "\nSabtu : " + tgel[a].Sabtu
                gel += "\nMinggu : " + tgel[a].Minggu
            }
            await tobz.reply(from, gel, id)
            break
        case prefix+'autosticker':
            if (args[1] === 'enable') {
                if (isAutoStickerOn) return await tobz.reply(from, 'Fitur auto stiker sudah diaktifkan,,, jika kamu kirom foto di group ini maka akan otomatis menjadi sticker..', id)
                _autosticker.push(chat.id)
                fs.writeFileSync('./lib/helper/antisticker.json', JSON.stringify(_autosticker))
                await tobz.reply(from, 'Fitur autosticker berhasil diaktifkan.. jika kamu kirim foto di group ini maka otomatis akan menjadi sticker' , id)
            } else if (args[1] === 'disable') {
                _autosticker.splice(chat.id, 1)
                fs.writeFileSync('./lib/helper/antisticker.json', JSON.stringify(_autosticker))
                await tobz.reply(from, 'Fitur autostiker berhasil dinonaktifkan' , id)
            } else {
